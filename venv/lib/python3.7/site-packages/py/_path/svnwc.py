...
class RepoCache:
    ...
    timeout = (
        20  # seconds after which we forget that we know the last revision
    )
...
def checkbadchars(url):
    ...
    if proto != "file":
        ...
        if _check_for_bad_chars(
            host, ALLOWED_CHARS_HOST
...
class SvnWCCommandPath(common.PathBase):
    ...
    def status(self, updates=0, rec=0, externals=0):
        ...
        if externals:
            raise ValueError(
                "XXX cannot perform status() " "on external items yet"
            )
        else:
...
def make_recursive_propdict(
    wcroot, output, rex=re.compile("Properties on '(.*)':")
):
