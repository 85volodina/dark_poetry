    ...
    class TextIO(StringIO):
        def write(self, data):
            if not isinstance(data, unicode):
                data = unicode(
                    data, getattr(self, "_encoding", "UTF-8"), "replace"
                )
...
class StdCaptureFD(Capture):
    ...
    def _save(self):
        ...
        if in_:
            try:
                self.in_ = FDCapture(
                    0, tmpfile=None, now=False, patchsys=patchsys
                )
            except OSError:
                pass
        if out:
            ...
            try:
                self.out = FDCapture(
                    1, tmpfile=tmpfile, now=False, patchsys=patchsys
                )
        ...
        if err:
            ...
            try:
                self.err = FDCapture(
                    2, tmpfile=tmpfile, now=False, patchsys=patchsys
                )
