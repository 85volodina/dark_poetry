...
class Engine(EngineBase):
    ...
    def _orientation(self, image):
        ...
        if orientation:
            if orientation == 2:
                ...
                image = image.rotate(-90, expand=1).transpose(
    ...
    def _remove_border(self, image, image_width, image_height):
        ...
        for border in ["top", "bottom"]:
            ...
            while offset[border] < image_height / 3.5:
                ...
                y_range = borders[border](
                    image_height, offset[border], slice_size
                )
        ...
        for border in ["left", "right"]:
            while offset[border] < image_width / 3.5:
                ...
                x_range = borders[border](
                    image_width, offset[border], slice_size
                )
    ...
    def _entropy_crop(
        self, image, geometry_width, geometry_height, image_width, image_height
    ):
        ...
        while image_width / image_height > geometry_ratio:
            ...
            right = image.crop(
                (image_width - slice_width, 0, image_width, image_height)
            )
            ...
            if self._get_image_entropy(left) < self._get_image_entropy(right):
                ...
                image = image.crop(
                    (0, 0, image_height - slice_width, image_height)
                )
        ...
        while image_width / image_height < geometry_ratio:
            ...
            bottom = image.crop(
                (0, image_height - slice_height, image_width, image_height)
            )
            top = image.crop((0, 0, image_width, slice_height))

            if self._get_image_entropy(bottom) < self._get_image_entropy(top):
                image = image.crop(
                    (0, 0, image_width, image_height - slice_height)
                )
            else:
                image = image.crop(
                    (0, slice_height, image_width, image_height)
                )
