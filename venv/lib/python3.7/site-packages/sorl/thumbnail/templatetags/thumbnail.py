        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if sorl_settings.THUMBNAIL_DEBUG:
                    raise
                logger.error(
                    "Thumbnail filter failed: %s" % str(err),
            ...
            if context.template.engine.debug:
                try:
                    ...
                    error_message = error_message_template.format(
                        template_name=template_origin.name, tag_text=tag_text,
                    )
                except Exception:
                    pass
...
class ThumbnailNode(ThumbnailNodeBase):
    ...
    error_msg = (
        "Syntax error. Expected: ``thumbnail source geometry "
    ...
    def _render(self, context):
        ...
        if not thumbnail or (
            isinstance(thumbnail, DummyImageFile) and self.nodelist_empty
        ):
...
def resolution(file_, resolution_string):
    ...
    if sorl_settings.THUMBNAIL_DUMMY:
        ...
        try:
            ...
            info = {
                dimension: int(int(size) * resolution)
                for (dimension, size) in info.items()
            }
...
def margin(file_, geometry_string):
    ...
    if not file_ or (
        sorl_settings.THUMBNAIL_DUMMY or isinstance(file_, DummyImageFile)
    ):
        return "auto"
...
def html_thumbnails(value):
    return text_filter(
        r'<img(?: alt="(%(re_cap)s)?")? src="(%(re_img)s)"', value
    )
