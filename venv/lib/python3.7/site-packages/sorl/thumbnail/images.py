...
from sorl.thumbnail.helpers import (
    ThumbnailError,
    deserialize,
    get_module_class,
    tokey,
...
def serialize_image_file(image_file):
    if image_file.size is None:
        raise ThumbnailError(
            "Trying to serialize an ``ImageFile`` with a " "``None`` size."
        )
    data = {
...
class DummyImageFile(BaseImageFile):
    def __init__(self, geometry_string):
        self.size = parse_geometry(
            geometry_string, settings.THUMBNAIL_DUMMY_RATIO,
        )
...
class UrlStorage(Storage):
    ...
    def open(self, name, mode="rb"):
        ...
        user_agent = "python-urllib{python_version}/0.6".format(
